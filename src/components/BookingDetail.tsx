...
export default function BookingDetail({ id }: { id: string }) {
  ...
  useEffect(() => {
    api.getBooking(id).then((result) => {
      if (result.ok) setBooking(result.data);
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <dl className="mt-4 space-y-3 text-sm">
              ...
              <Row icon={<UsersIcon className="h-4 w-4" />} label="Huespedes" value={String(booking.guests)} />
              <Row icon={<CalendarIcon className="h-4 w-4" />} label="Noches" value={String(booking.nights)} />
            </dl>
            <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-4">
              <span className="font-semibold text-gray-900">Total</span>
              <span className="text-xl font-bold text-gray-900">
                {formatPrice(booking.totalPrice, property?.currency)}
              </span>
            </div>
          </div>
