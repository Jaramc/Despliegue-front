...
export default function PropertyMap({
  ...
  const center = useMemo<[number, number]>(() => {
    ...
    const lat = valid.reduce((sum, p) => sum + (p.latitude ?? 0), 0) / valid.length;
    const lng = valid.reduce((sum, p) => sum + (p.longitude ?? 0), 0) / valid.length;
    return [lat, lng];
  }, [properties]);
        ...
        {properties
          ...
          .map((property) => (
            <Marker
              ...
              icon={priceIcon(formatPrice(property.nightlyRate))}
            >
              <Popup>
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-medium text-gray-800">
                    {formatPrice(property.nightlyRate)} / noche
                  </p>
                  <Link
